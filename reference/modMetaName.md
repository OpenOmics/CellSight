...
on the shiny app. This function allows users to specify display names
for metadata i.e. the names that will be displayed on the shiny app.
Note that `showLegend` shows the display name instead of the actual
name.
...
- meta.to.mod:
  ...
  metadata for which to modify the display name. Users can either use
  the actual metadata column names or display names. Multiple metadata
  can be specified. It is reccomended to use the original metadata
