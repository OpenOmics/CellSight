...
prompt_for_install <- function(pkg) {
  ...
  if (response == "y") {
    if (pkg == "cellsight") {
      remotes::install_github("OpenOmics/CellSight", quiet = TRUE)
    } else {
...
if (!suppressPackageStartupMessages(require("cellsight", quietly = TRUE))) prompt_for_install("cellsight")
...
parser$add_argument(
  "--rmmeta",
  dest = "meta.to.rm",
  help = "Comma delimited list of names in seurat_object@meta.data that will be dropped prior to deploying to CellSight",
  default = NULL,
...
parser$add_argument("--defred",
  ...
  help = paste0("The default reduction to use with CellSight, this value must exist in Seurat::DefaultDimReduc(obj).\n
...
parser$add_argument(
  ...
  help = "The assay to utilize for CellSight web application. \n
...
# setup opt parse variables for downstream usage into CellSight
...
# A mismatch would corrupt the CellSight data files, which assume cell ordering
...
# CellSight building / the Shiny app expect a 'sample' column to be present.
...
cellsight_config <- do.call(
  createConfig,
  c(seurat_obj, config_params)
)

# if (!is.null(length(meta_to_include) > 0)) {
#   cellsight_config$meta.to.del <- remove_metas
#   cellsight_config <- delMeta(cellsight_config, remove_metas)
# }
...
if (!args$codes.only) {
  start_time <- Sys.time()
  cat(paste0("INFO - ", start_time, " - start building CellSight files\n"))
  do.call(
    makeShinyFiles,
    files_params
  )
  end_time <- Sys.time()
  cat(paste0("INFO - ", end_time, " - stop building CellSight files\n"))
  cat(paste0("INFO - Total time taken to build CellSight files: ", round(difftime(end_time, start_time, units = "mins"), 2), " minutes\n"))
}
cat("\n")
if (!args$files.only) {
  start_time <- Sys.time()
  cat(paste0("INFO - ", start_time, " - start building CellSight codes\n"))
  makeShinyCodes(
  ...
  end_time <- Sys.time()
  cat(paste0("INFO - ", end_time, " - stop building CellSight codes\n"))
  cat(paste0("INFO - Total time taken to build CellSight codes: ", round(difftime(end_time, start_time, units = "mins"), 2), " minutes\n"))
}
